...
* Preferred scenario for growing the Infrastructure and making Puppet server highly available.
* But all Puppet servers depend on Foreman so place it on a highly available machine.
...
system. Last run the Foreman installer on the systems with parameters to disable Foreman
and the Puppet CA.
Depending on your infrastructure there are different ways to balance the load on the Puppet
servers:

* Manually point servers to a Puppet server
...
* Foreman installer helps to set up Smart Proxies
...
This could be done on the Foreman host itself or any other host from which communication is possible.
...
Installation of PuppetDB, if required for your setup, can easily be done with the Puppet
...
* We talk about Remote Execution in more detail later
...
* Katello adds additional complexity, so the level of detail is reduced to the main components
